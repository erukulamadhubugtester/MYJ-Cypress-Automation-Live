...
const phoneWithout91 = "1234567890";
...
cy.get(".form-control").then(($input) => {
  ...
  if (existingValue?.startsWith("91")) {
    cy.wrap($input)
      .clear()
      .type(phoneWithout91, { delay: 300, log: false }); // only 10-digit if already has 91
  } else {
    cy.wrap($input)
      .clear()
      .type(phoneWith91, { delay: 300, log: false }); // full number with 91
  }
});
