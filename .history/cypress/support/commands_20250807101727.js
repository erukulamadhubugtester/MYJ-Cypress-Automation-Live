...
cy.get(".form-control").then(($input) => {
  ...
  const existingValue = rawValue ? rawValue.toString().trim() : "";

  const has91Prefix = existingValue.startsWith("91");
  const numberToType = has91Prefix ? phoneWithout91 : phoneWith91;

  cy.wrap($input)
    .clear()
    .type(numberToType, { delay: 300, log: false });

  // Optional debug
  // cy.log(`Existing: ${existingValue}, Typing: ${numberToType}`);
});
