...
Cypress.Commands.add("loginAndHandlePopup", () => {
  ...
  // ✅ Safely handle phone input and avoid duplicate '91'
cy.get(".form-control").then(($input) => {
  const rawValue = $input.val() || "";
  const existingValue = rawValue.toString().replace(/\s/g, "").trim();

  // If value already includes '91' at the beginning, don't add it again
  const needs91 = !existingValue.startsWith("91");
  const numberToType = needs91 ? phoneWith91 : phoneWithout91;

  // Log for debugging
  cy.log(`Input has: "${existingValue}", typing: "${numberToType}"`);
