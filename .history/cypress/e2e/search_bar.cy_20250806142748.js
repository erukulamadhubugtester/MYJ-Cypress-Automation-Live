describe("MakeYourJodi - User Search Page Checking ", () => {
  ...
  it("Search ", () => {
    ...
    cy.get(".h-7", { timeout: 10000 })
      ...
      .invoke("text")
      .then((rawId) => {
        profileIdText = rawId.trim();

        // Log it regardless of empty/non-empty for debug
        cy.log("Raw Profile ID: '" + profileIdText + "'");

        // Assert only if it's not empty
        if (!profileIdText) {
          throw new Error(
            "❌ Profile ID text is empty. Selector '.h-7' may not be valid or loaded."
          );
        }

        cy.log("✅ Profile ID captured: " + profileIdText);
      });
