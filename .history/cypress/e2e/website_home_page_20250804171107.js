describe("MakeYourJodi UI Smoke Test", () => {
  ...
  it("verifies 'Why MakeYourJodi' info tiles", () => {
    ...
    cy.get(".flex.mt-6 > .flex > .text-\\[\\#AB1D79\\]").should('contain.text', 'Smart Matches');
  ...
  it("verifies social media links", () => {
    cy.get('[href*="linkedin"]').should('have.attr', 'href').and('include', 'linkedin');
    cy.get('[href*="facebook"]').should('have.attr', 'href').and('include', 'facebook');
    cy.get('[href*="instagram"]').should('have.attr', 'href').and('include', 'instagram');
    cy.get('[href*="x.com"]').should('have.attr', 'href').and('include', 'x.com');
  });
