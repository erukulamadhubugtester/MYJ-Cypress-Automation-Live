describe("MakeYourJodi - User Search Page Checking ", () => {
  ...
  it("Search ", () => {
    ...
    cy.wait(2000); // or better: wait for profile container to be visible
    cy.get(".w-\\[90\\%\\].mx-auto.grid").should("be.visible");

    // 5. Click on the first profile card
    cy.get(':nth-child(1) > .gap-8 > .w-\[14rem\]').should("exist").click();
