// /cypress/e2e/ui-smoke.cy.ts

describe("MakeYourJodi UI Smoke Test", () => {
  const highlight = (selector: any) => {
    cy.get(selector)
      .scrollIntoView({ block: "center" })
      .should("be.visible")
      .then(($el) => {
        $el[0].style.border = "3px solid red";
        $el[0].style.backgroundColor = "rgba(255, 255, 0, 0.3)"; // light yellow
      });
  };

  it("verifies and highlights key elements", () => {
