describe("MakeYourJodi UI Smoke Test", () => {
  it("verifies key elements are present", () => {
    ...
    cy.get(".w-\\[200px\\]").scrollIntoView({ block: "center" });
    cy.get(".min-w-\\[280px\\] > .mt-5").scrollIntoView({ block: "center" });
    cy.get(
    ...
    cy.get('[href="https://x.com/MakeYourJodi"] > .h-\\[30px\\]').scrollIntoView({ block: "center" });
    cy.get(".h-\\[50px\\]").scrollIntoView({ block: "center" });
    cy.get(".flex-\\[2\\] > .font-semibold").scrollIntoView({ block: "center" });
    cy.get(".flex > .mt-5 > :nth-child(1) > .transition-all").scrollIntoView({ block: "center" });
    cy.get(".mt-5 > :nth-child(2) > .transition-all").scrollIntoView({ block: "center" });
    cy.get(".sm\\:mt-5 > :nth-child(1) > .transition-all").scrollIntoView({ block: "center" });
    cy.get(".sm\\:mt-5 > :nth-child(2) > .transition-all").scrollIntoView({ block: "center" });
    cy.get(".flex-2 > .font-semibold").scrollIntoView({ block: "center" });
    cy.get(".flex-2 > .mt-5 > .mb-2 > .transition-all").scrollIntoView({ block: "center" });
    cy.get(".xl\\:border-l-\\[0\\.5px\\] > .flex-col").scrollIntoView({ block: "center" });
    cy.get(".mt-6 > span");
  });
});
